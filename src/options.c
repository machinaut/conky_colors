...
int options (int argc, char *argv[]) {
	...
	while (--argc) {
		...
		OPTION("--theme", key) {
			OR_OPTION_START("gnome-brave", value) OR_OPTION("gnome-carbonite", value) OR_OPTION("gnome-dust", value)
			OR_OPTION("gnome-human", value) OR_OPTION("gnome-illustrious", value) OR_OPTION("gnome-noble", value)
			OR_OPTION("gnome-tribute", value) OR_OPTION("gnome-wine", value) OR_OPTION_END("gnome-wise", value)
			{
				snprintf(theme, 31, "%s", value);
			}
			else OR_OPTION_START("shiki-brave", value) OR_OPTION("shiki-human", value) OR_OPTION("shiki-illustrious", value)
			OR_OPTION("shiki-noble", value) OR_OPTION("shiki-wine", value) OR_OPTION_END("shiki-wise", value)
			{
				snprintf(theme, 31, "%s", value);
				shiki = True;
				go2 = 38;
			}
			else OPTION("shiki-dust", value)
			{
				snprintf(theme, 31, "%s", value);
				shikidust = True;
				go2 = 38;
			}
			else OPTION("dust", value)
			{
				snprintf(theme, 31, "%s", value);
				dust = True;
				go2 = 38;
		...
		else OPTION_WITH_VALUE("--hdtemp4", key)
		{
			...
			covergloobus = True;
		else OR_OPTION_START("--rhythmbox", key) OR_OPTION("--exaile", key) OR_OPTION_END("--banshee", key)
		{
			...
			{
				printf("ERR0: option %s unavailable", value);
				return OPTIONS_ERROR;
			}
		}
