...
function conky_widgets(color, theme, n_cpu, cputype, swap, clock_theme, player, player_theme, shapes, wireless, wlan)
	...
	-- PLAYERS
	if player == "Banshee" or player == "Rhythmbox" or player == "Exaile" then
		if player_theme == "cairo" then
			ring(player, '', 100, bgc, bga, fgc, fga, 154, ring_pos, 10, 20, 0, 360, 1)
			ring(player, '', 100, bgc, 0.4, fgc, 0.2, 154, ring_pos, 20, 1, 0, 360, 1)
		elseif player_theme == "cairo-cover" then
			ring(player, '', 100, bgc, 0.4, theme, 1, 174, ring_pos-18, 5, 11, 0, 360, 1)
			ring(player, '', 100, bgc, 0.6, fgc, 0.2, 174, ring_pos-18, 11, 2, 0, 360, 1)
		else
	...
	if wireless == "on" then
		ring_pos = ring_pos + 64
		ring('wireless_link_qual', wlan, 100, bgc, bga, fgc, fga, 190, ring_pos, 20, 10, 180, 360, 0)
	end
	cairo_destroy(cr)
end
